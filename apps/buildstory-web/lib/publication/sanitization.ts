...
const rules: ReplacementRule[] = [
  ...
  { pattern: /\b(?!git@)[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,63}\b/gi, marker: "email-address" },
  { pattern: /\b[a-z][a-z0-9+.-]*:\/\/[^\s<>()]+/gi, marker: "remote-url" },
  { pattern: /\bgit@[^\s:]+:[^\s]+/gi, marker: "remote-url" },
  { pattern: /(?:^|[\s(=])(?:"(?:[A-Za-z]:[\\/]|\\\\|\/(?!\/))[^"\r\n]+"|'(?:[A-Za-z]:[\\/]|\\\\|\/(?!\/))[^'\r\n]+'|`(?:[A-Za-z]:[\\/]|\\\\|\/(?!\/))[^`\r\n]+`)/g, marker: "absolute-path" },
  { pattern: /(?:^|[\s(=])(?:"(?:\.\.?[\\/]|~[\\/])[^"\r\n]+"|'(?:\.\.?[\\/]|~[\\/])[^'\r\n]+'|`(?:\.\.?[\\/]|~[\\/])[^`\r\n]+`)/g, marker: "relative-path" },
  ...
  { pattern: /\b(?:[a-z0-9-]+\.)+(?:com|net|org|io|dev|app|co|ai|cloud|tech|local|internal|invalid|test|example|ca|uk|xyz|me|info|biz|us|eu|de|fr|jp|au|in)(?::\d{1,5})?\b/gi, marker: "raw-host" },
