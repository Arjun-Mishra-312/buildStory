...
test("anonymous visitors see the full primary navigation, including discovery links", async () => {
  ...
  assert.match(primaryNav, /Badges/);
...
test("landing page leads with private report generation and evidence-backed facts", async () => {
  ...
  assert.match(html, /npx buildstory-scan generate --repo \. --consent local-scan/);
  ...
  assert.match(html, /Which kind of builder was this session/);
  assert.match(html, /Named receipts for the work/);
  assert.match(html, /id="badges"/);
  assert.match(html, /id="builder"/);
  assert.match(html, /id="board"/);
  assert.match(html, /Dawn Watch/);
  ...
  assert.match(html, /open it on BuildStory/);
  assert.match(html, /class="github-star"/);
  assert.match(html, /href="https:\/\/github.com\/Arjun-Mishra-312\/buildstory-scan"/);
  assert.doesNotMatch(html, /Generate my report/);
  assert.doesNotMatch(html, /npx buildstory(?!-scan)/);
