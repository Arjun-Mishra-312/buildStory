...
export function CommentThread({ storyId, chapterCount = 1 }: { storyId: string; chapterCount?: number }) {
  ...
  const renderComment = (comment: Comment, reply = false) => { const upvoted = viewer.upvotedCommentIds.includes(comment.id); const removed = comment.status !== "visible"; const removedLabel = comment.status === "hidden" ? "This comment was removed by a moderator." : "This comment was removed."; const profileHref = `/u/${comment.author.handle}`; return <article className={`comment-card${reply ? " comment-card--reply" : ""}`} key={comment.id}>{removed ? <div className="comment-card__avatar">{comment.author.displayName.slice(0, 1).toUpperCase()}</div> : <Link className="comment-card__avatar" href={profileHref} aria-label={`View ${comment.author.displayName}'s profile`}>{comment.author.displayName.slice(0, 1).toUpperCase()}</Link>}<div className="comment-card__body"><header><div><strong>{removed ? "Deleted comment" : <Link className="comment-card__author" href={profileHref}>{comment.author.displayName}</Link>}</strong><span>{removed ? null : <>@{comment.author.handle} · </>}{relativeTime(comment.createdAt)}{chapterCount > 1 ? ` · Ch. ${comment.chapterIndex}` : ""}</span></div>{!removed ? <details className="comment-card__menu"><summary aria-label="Comment actions">•••</summary><div><ReportDialog targetType="comment" targetId={comment.id} />{viewer.removableCommentIds.includes(comment.id) ? <button type="button" onClick={() => void remove(comment.id)}>Remove</button> : null}</div></details> : null}</header><p>{removed ? removedLabel : comment.body}</p>{!removed ? <div className="comment-card__actions"><button type="button" onClick={() => void toggleUpvote(comment)} aria-pressed={upvoted} className={upvoted ? "is-active" : ""}>Upvote <strong>{comment.upvoteCount}</strong></button>{!reply ? <button type="button" onClick={() => setReplyTo(replyTo === comment.id ? null : comment.id)}>Reply</button> : null}</div> : null}</div></article>; };
