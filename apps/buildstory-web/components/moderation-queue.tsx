...
const ACTION_LABEL: Record<ContentReportRecord["targetType"], string> = {
  report: "Hide story",
...
export function ModerationQueue({ initialReports }: { initialReports: ContentReportRecord[] }) {
  ...
  return <div className="moderation-queue">{reports.length === 0 ? <p>No reports in this queue.</p> : reports.map((report) => <article className="report-card" key={report.id}><span className="section-index">{report.targetType} · {report.reasonCode}</span><h2>{report.targetId}</h2>{report.note ? <p>{report.note}</p> : null}<small>{new Date(report.createdAt).toLocaleString()}</small>{report.status === "open" ? <div><button className="button button--primary button--small" onClick={() => void resolve(report.id, "actioned")}>{ACTION_LABEL[report.targetType]}</button><button className="button button--secondary button--small" onClick={() => void resolve(report.id, "dismissed")}>Dismiss</button></div> : <strong>{report.status}</strong>}</article>)}</div>;
}
